...
/** 加 进度条，  */
export const getWbpackBarPlugins = (
  conf: WebpackConfiguration,
  opt: WebpackBar['options'],
...
export const WebpackManifestPluginGenerate = ({
  seed,
  files,
  entrypoints,
  isCreateAsset,
  ...
  paths,
  type,
}: WebpackManifestPluginGenerateProps): Manifest => {
  ...
  const entrypointFiles = entrypoints.main
    .filter(
      (fileName) =>
        !(fileName.endsWith('.map') || fileName.endsWith('.hot-update.js')),
    )
  ...
  if (isCreateAsset) {
    ...
    const outPath = type
      ? `${paths.appBuild}/asset-${type}-manifest.json`
      : `${paths.appBuild}/asset-manifest.json`;
...
export const addServerPlugins = (
  conf: WebpackConfiguration,
  out: webpack.Configuration['output'],
) => {
  conf.plugins.push(
    /**  生成 server.js 文件  */
...
export const AddDefinePlugin = (
  ...
  const define = {
    ...
    KKT_PUBLIC_DIR: JSON.stringify(
      process.env.KKT_PUBLIC_DIR || overrides.output_path,
    ),
    /** 当前ip地址 **/
  ...
  conf.plugins.push(new webpack.DefinePlugin(define));
  return conf;
};
